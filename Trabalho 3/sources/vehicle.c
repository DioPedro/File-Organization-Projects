...
void read_vehicle_bin(FILE *bin_fp){
    ...
    for (int i = 0; i < num_of_register; i++){
        ...
        if (!should_read){
            ...
            print_vehicle_register(&cur_register, TRUE);
            free_vehicle_strings(&cur_register);
        }
    }
}
...
void search_vehicle_by_field(FILE *bin_fp, char *field, char *value){
    ...
    for (int i = 0; i < header.num_of_regs; i++){
        ...
        if (are_equal){
            ...
            print_vehicle_register(&valid_register, TRUE);
            was_printed = TRUE;

            free_vehicle_strings(&valid_register);
        } else {
